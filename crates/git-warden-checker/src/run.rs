...
pub fn run_encoding(
    ...
    for_each_file_concurrent(files, cancel, |path| {
        ...
        if let Some(def) = git_warden_editorconfig::get_definition(path) {
        ...
        let result = git_warden_encoding::check_utf8(&content);
        if !result.valid {
            return Ok(vec![git_warden_i18n::t!(
                "diff.encoding_error",
                Path = path,
...
pub fn run_editorconfig(
    ...
    for_each_file_concurrent(files, cancel, |path| {
        if git_warden_pathutil::matches_any(path, &cfg.exceptions.global_ignore) {
            return Ok(Vec::new());
        }
        if git_warden_pathutil::matches_any(path, &cfg.editorconfig.ignore_files) {
            return Ok(Vec::new());
        }
        let def = match git_warden_editorconfig::get_definition(path) {
            Some(d) => d,
        ...
        let msgs = git_warden_editorconfig::check(path, &content, &def)
            .into_iter()
            .map(|v| {
                git_warden_i18n::t!(
...
pub fn run_comment_language(
    ...
    let extensions = if !cfg.comment_language.languages.is_empty() {
        git_warden_comment::extensions_for_languages(&cfg.comment_language.languages)
    } else {
    ...
    let kind_comment = git_warden_i18n::t!("diff.kind_comment");

    for_each_file_concurrent(files, cancel, |file_path| {
        if !git_warden_gitdiff::has_extension(file_path, &extensions) {
            return Ok(Vec::new());
        }
        if git_warden_pathutil::matches_any(file_path, &ignore_patterns) {
            return Ok(Vec::new());
        }
        let parser = match git_warden_comment::get_parser(file_path) {
            Some(p) => p,
            None => return Ok(Vec::new()),
        };
        ...
        let states = git_warden_directive::analyze(&comments, &file_lang);

        let mut msgs = Vec::new();
        for u in build_comment_units(&comments, &states, check_strings) {
            if u.kind == git_warden_comment::Kind::String {
                continue;
            }
            let text = git_warden_langdetect::strip_allowed_words(&u.text, &allowed_words);
            let (ok, has_content) = git_warden_langdetect::is_required_language(
                &text,
                &u.lang,
                min_length,
                &skip_directives,
            ...
            if !ok {
                let detected = git_warden_langdetect::dominant_language(&text);
                msgs.push(git_warden_i18n::t!(
            ...
            if no_emoji {
                for e in git_warden_emoji::find_emojis(&text) {
                    msgs.push(git_warden_i18n::t!(
                        "diff.emoji_error",
                        Path = file_path,
