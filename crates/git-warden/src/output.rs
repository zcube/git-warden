...
use crate::progress::{format_json, run_with_progress, summary, Options, Step, StepResult};
...
fn guide_text(category: &str) -> String {
    ...
    let text = git_warden_core::i18n::translate(&key, &[]);
    if text == format!("[{key}]") {
        String::new()
    } else {
        text
...
fn print_guides(categories: &[String], guides: &BTreeMap<String, String>) {
    ...
    eprintln!();
    eprintln!("{}", git_warden_core::t!("guide.header"));
...
pub fn run_steps_and_report(
    ...
    if !result.all_errors.is_empty() {
        ...
        if total > 0 {
            eprintln!(
                "{}",
                git_warden_core::t!("summary.violations", Count = total, Checks = checks)
            );
        }
        if with_guide {
