...
pub fn check_append_only(
    ...
    for d in diffs {
        ...
        if !crate::pathutil::matches_any(&d.path, &cfg.append_only.paths) {
            continue;
        }
        ...
        if d.is_deleted {
            errs.push(crate::t!("diff.append_only_deleted", Path = d.path));
            continue;
        }
...
fn list_tree_files(tree_ref: &str) -> Vec<String> {
    ...
    crate::gitdiff::split_null_separated(&out)
}
...
fn check_filename_order(tree_ref: &str, new_path: &str, patterns: &[String]) -> Option<String> {
    ...
    for name in list_tree_files(tree_ref) {
        ...
        if !crate::pathutil::matches_any(&name, patterns) {
            continue;
        }
...
fn check_file_content(tree_ref: &str, file_path: &str) -> Result<Option<String>, String> {
    ...
    let staged_content = crate::gitdiff::get_staged_content(file_path)?;
