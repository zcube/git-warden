...
//! `cc_gitdiff::get_staged_content` returns a `String` (lossy-decoded), which loses invalid
...
/// Byte-level version of cc_gitdiff::get_content_at (no cache).
...
fn content_at_bytes(reference: &str, path: &str) -> Result<Vec<u8>, String> {
    match reference {
        ...
        cc_gitdiff::REF_WORKTREE | "wt" | "working-tree" => {
...
pub(crate) fn get_staged_binary_files() -> Result<Vec<String>, String> {
    ...
    let fields = cc_gitdiff::split_null_separated(&out.stdout);
