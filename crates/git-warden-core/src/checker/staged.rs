...
//! `crate::gitdiff::get_staged_content` returns a `String` (lossy-decoded), which loses invalid
...
/// Byte-level version of crate::gitdiff::get_content_at (no cache).
pub(crate) fn staged_content_bytes(path: &str) -> Result<Vec<u8>, String> {
    let spec = crate::gitdiff::current_spec();
...
fn content_at_bytes(reference: &str, path: &str) -> Result<Vec<u8>, String> {
    match reference {
        ...
        crate::gitdiff::REF_WORKTREE | "wt" | "working-tree" => {
...
pub(crate) fn get_staged_files() -> Result<Vec<String>, String> {
    ...
    Ok(crate::gitdiff::split_null_separated(&out.stdout))
}
...
pub(crate) fn get_staged_binary_files() -> Result<Vec<String>, String> {
    ...
    let fields = crate::gitdiff::split_null_separated(&out.stdout);
