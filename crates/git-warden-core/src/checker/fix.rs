...
use crate::config::{CommitMessageConfig, Config};

use super::msg::decode_rune;
...
pub fn fix_file_content(cfg: &Config, content: &[u8]) -> FixResult {
    ...
    if cfg.encoding.is_no_ambiguous_chars() {
        let tables = crate::charset::tables_for_locale(&cfg.commit_message.locale);
...
fn fix_ambiguous_chars(
    ...
    tables: &[&crate::charset::AmbiguousTable],
) -> String {
    let mut sb = String::with_capacity(content.len());
    for (line_idx, line) in content.split('\n').enumerate() {
        if line_idx > 0 {
