...
use git_warden_config::{CommitMessageConfig, CommitMessageLanguageConfig, Config};

use crate::conventional::check_conventional;
use crate::custom::check_msg_custom_rules;
use crate::diff_check::truncate;
...
pub fn check_msg(cfg: &Config, content: &[u8]) -> Vec<String> {
    ...
    if cm.is_no_ambiguous_chars() {
        let tables = git_warden_charset::tables_for_locale(&cm.locale);
...
fn check_coauthor(content: &str, cfg: &CommitMessageConfig) -> Vec<String> {
    ...
    for (i, line) in content.split('\n').enumerate() {
        ...
        let email = git_warden_config::extract_coauthor_email(trimmed);
...
fn check_invisible_chars(content: &str) -> Vec<String> {
    ...
    for (line_idx, line) in content.split('\n').enumerate() {
        ...
        for r in line.chars() {
            col += 1;
            if git_warden_charset::is_invisible(r) {
                let name = git_warden_charset::invisible_name(r);
...
fn check_ambiguous_chars(
    content: &str,
    tables: &[&git_warden_charset::AmbiguousTable],
) -> Vec<String> {
    let mut errs = Vec::new();
    for (line_idx, line) in content.split('\n').enumerate() {
        let mut col = 0;
        for r in line.chars() {
            col += 1;
            if let Some(confusable_to) = git_warden_charset::is_ambiguous(r, tables) {
                errs.push(git_warden_i18n::t!(
...
fn check_msg_language(content: &str, cfg: &CommitMessageLanguageConfig) -> Vec<String> {
    ...
    let mut check_line = |line_num: usize, text: &str| {
        let text = text.trim();
        let violated = if required == git_warden_langdetect::ENGLISH {
            // English-required: any CJK / Hangul / Kana character is not allowed.
            if !git_warden_langdetect::has_natural_language_content(text, min_length, &[]) {
                false
            } else {
                git_warden_langdetect::has_script(text, git_warden_langdetect::KOREAN)
                    || git_warden_langdetect::has_script(text, git_warden_langdetect::JAPANESE)
                    || git_warden_langdetect::has_script(text, git_warden_langdetect::CHINESE)
            }
        } else {
            // Korean / Japanese / Chinese / any: presence-based; English mixed in is allowed.
            let (ok, has_content) =
                git_warden_langdetect::is_required_language(text, &required, min_length, &[]);
            has_content && !ok
        };
        if violated {
            errs.push(git_warden_i18n::t!(
                ...
                Line = line_num,
                Language = required,
                Detected = git_warden_langdetect::dominant_language(text),
...
fn check_msg_emoji(content: &str) -> Vec<String> {
    git_warden_emoji::find_emojis(content)
        .into_iter()
        .map(|e| {
            git_warden_i18n::t!(
                "msg.emoji_error",
