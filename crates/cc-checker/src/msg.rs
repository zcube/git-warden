...
use cc_config::{CommitMessageConfig, CommitMessageLanguageConfig, Config};
...
fn check_ambiguous_chars(content: &str, tables: &[&cc_charset::AmbiguousTable]) -> Vec<String> {
    let mut errs = Vec::new();
    for (line_idx, line) in content.split('\n').enumerate() {
        let mut col = 0;
        for r in line.chars() {
...
fn check_msg_language(content: &str, cfg: &CommitMessageLanguageConfig) -> Vec<String> {
    ...
    let mut check_line = |line_num: usize, text: &str| {
        let text = text.trim();
        let violated = if required == cc_langdetect::ENGLISH {
            // English-required: any CJK / Hangul / Kana character is not allowed.
            if !cc_langdetect::has_natural_language_content(text, min_length, &[]) {
                false
            } else {
                cc_langdetect::has_script(text, cc_langdetect::KOREAN)
                    || cc_langdetect::has_script(text, cc_langdetect::JAPANESE)
                    || cc_langdetect::has_script(text, cc_langdetect::CHINESE)
            }
        } else {
            // Korean / Japanese / Chinese / any: presence-based; English mixed in is allowed.
            let (ok, has_content) =
                cc_langdetect::is_required_language(text, &required, min_length, &[]);
            has_content && !ok
        };
        if violated {
            errs.push(cc_i18n::t!(
                ...
                Line = line_num,
                Language = required,
                Detected = cc_langdetect::dominant_language(text),
