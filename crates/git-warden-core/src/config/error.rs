...
pub fn format_config_error(cfg_path: &str, err: &str) -> String {
    ...
    if !err.contains("invalid type:") {
        return crate::t!("config.syntax_error", Path = cfg_path, Error = err);
    }
    ...
    let mut out = crate::t!("config.type_error_header", Path = cfg_path);
    out.push('\n');
    ...
    match expected.clone().and_then(|s| type_hint(&s)) {
        Some(hint) => {
            ...
            out.push('\n');
            out.push_str(&crate::t!("config.type_error_example_header"));
            out.push('\n');
            for ex in hint.example.split('\n') {
                out.push_str(&crate::t!("config.type_error_example_line", Line = ex));
                out.push('\n');
            }
        }
        None => {
