...
use crate::lint::ValidationError;

use super::staged::get_staged_files;
...
fn lint_validate(cfg: &Config, path: &str, content: &str) -> Vec<ValidationError> {
    match ext_lower(path).as_str() {
        ".yaml" | ".yml" => {
            ...
            if crate::pathutil::matches_any(path, &cfg.lint.yaml.ignore_files) {
                return Vec::new();
            }
            if cfg.lint.yaml.is_comment_filter()
                && crate::lint::has_lint_disable_comment(content, "#")
            {
                return Vec::new();
            }
            crate::lint::validate_yaml(path, content)
        }
        ".jsonc" => {
            if !cfg.lint.json.is_enabled() {
                return Vec::new();
            }
            crate::lint::validate_json5(path, content)
        }
        ".json" => {
            if !cfg.lint.json.is_enabled() {
                return Vec::new();
            ...
            let ignored = if user_ignore.is_empty() {
                let defaults: Vec<String> = crate::lint::DEFAULT_JSON_IGNORE_FILES
                    .iter()
                    .map(|s| s.to_string())
                    .collect();
                crate::pathutil::matches_any(path, &defaults)
            } else {
                crate::pathutil::matches_any(path, user_ignore)
            };
            if ignored {
                return Vec::new();
            }
            if cfg.lint.json.is_allow_json5() {
                ...
                crate::lint::validate_jsonc(path, content)
            } else {
                crate::lint::validate_json(path, content)
            }
        }
        ".xml" => {
            ...
            if crate::pathutil::matches_any(path, &cfg.lint.xml.ignore_files) {
                return Vec::new();
            }
            crate::lint::validate_xml(path, content)
        }
        ".toml" => {
            if !cfg.lint.toml.is_enabled() {
                return Vec::new();
            }
            if crate::pathutil::matches_any(path, &cfg.lint.toml.ignore_files) {
                return Vec::new();
            }
            crate::lint::validate_toml(path, content)
        }
        _ => Vec::new(),
    }
}
...
pub fn check_lint(cfg: &Config, cancel: &AtomicBool) -> Result<Vec<String>, String> {
    ...
    for path in &files {
        ...
        if crate::pathutil::matches_any(path, global_ignore) {
            continue;
        }
        ...
        let content = match crate::gitdiff::get_staged_content(path) {
            Ok(c) => c,
            Err(_) => continue,
        };
...
pub fn run_lint(
    ...
    super::for_each_file_concurrent(files, cancel, |path| {
        if crate::pathutil::matches_any(path, &global_ignore) {
            return Ok(Vec::new());
        }
        if ext_lower(path).is_empty() {
