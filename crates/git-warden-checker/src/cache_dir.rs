...
pub fn check_cache_dir_staged(
    cfg: &Config,
    diffs: &[git_warden_gitdiff::FileDiff],
    cancel: &AtomicBool,
) -> Result<Vec<String>, String> {
    if !cfg.cache_dir.is_enabled() {
        return Ok(Vec::new());
    }
    let repo_root = match git_warden_cachedir::find_repo_root(Path::new(".")) {
        Ok(r) => r,
    ...
    for d in diffs {
        ...
        if git_warden_pathutil::matches_any(&d.path, &cfg.exceptions.global_ignore) {
            continue;
        }
        let abs_path = repo_root.join(&d.path);
        let cache_dir = match git_warden_cachedir::find_cache_dir_ancestor(&repo_root, &abs_path) {
            Some(c) => c,
            None => continue,
        };
...
pub fn check_cache_dir_committed(cfg: &Config, cancel: &AtomicBool) -> Result<Vec<String>, String> {
    ...
    let cache_dirs = git_warden_cachedir::find_cache_dirs_in_repo(&repo_root);
    ...
    for dir in &cache_dirs {
        ...
        let tracked = match git_warden_cachedir::list_tracked_entries(&repo_root, dir) {
            Ok(t) if !t.is_empty() => t,
            _ => continue,
        };
        let rel = rel_to(&repo_root, dir);
        if git_warden_pathutil::matches_any(&rel, &cfg.exceptions.global_ignore) {
            continue;
        }
        errs.push(git_warden_i18n::t!(
