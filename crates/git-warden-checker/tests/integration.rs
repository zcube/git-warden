...
use git_warden_config::{apply_defaults, Config, CustomRule};
...
fn append_only_new_file_allowed() {
    with_repo(|r| {
        ...
        let errs = git_warden_checker::check_append_only(&cfg, &diffs, &no_cancel()).unwrap();
...
fn protected_paths_modify_blocked() {
    with_repo(|r| {
        ...
        let diffs = gd::get_staged_diff().unwrap();
        let errs = git_warden_checker::check_protected_paths(&cfg, &diffs, &no_cancel()).unwrap();
...
fn binary_file_blocked_by_default() {
    with_repo(|r| {
        ...
        let errs = git_warden_checker::check_binary_files(&cfg, &no_cancel()).unwrap();
...
fn cache_dir_staged_blocked() {
    with_repo(|r| {
        ...
        let diffs = gd::get_staged_diff().unwrap();
        let errs = git_warden_checker::check_cache_dir_staged(&cfg, &diffs, &no_cancel()).unwrap();
        assert!(
            !errs.is_empty(),
...
fn custom_rules_diff_forbidden() {
    with_repo(|r| {
        ...
        let diffs = gd::get_staged_diff().unwrap();
        let errs = git_warden_checker::check_diff_custom_rules(&cfg, &diffs, &no_cancel()).unwrap();
...
fn comment_language_diff_english_flagged_korean_ok() {
    with_repo(|r| {
        ...
        let diffs = gd::get_staged_diff().unwrap();
        let errs = git_warden_checker::check_diff(&cfg, &diffs, &no_cancel()).unwrap();
...
fn encoding_non_utf8_flagged() {
    with_repo(|r| {
        ...
        let errs = git_warden_checker::check_encoding(&cfg, &no_cancel()).unwrap();
