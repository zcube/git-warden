...
pub fn apply_defaults(cfg: &mut Config) {
    ...
    {
        let mut lang = crate::langdetect::normalize_locale(&cfg.comment_language.locale);
        if lang.is_empty() {
            lang = crate::langdetect::normalize_locale(&cfg.comment_language.required_language);
        }
        if lang.is_empty() {
            lang = crate::langdetect::KOREAN.to_string();
        }
    ...
    for r in cfg.comment_language.file_languages.iter_mut() {
        let mut lang = crate::langdetect::normalize_locale(&r.locale);
        if lang.is_empty() {
            lang = crate::langdetect::normalize_locale(&r.language);
        }
        if !lang.is_empty() {
            r.locale = lang.clone();
            r.language = lang;
    ...
    {
        let mut lang =
            crate::langdetect::normalize_locale(&cfg.commit_message.language_check.locale);
        if lang.is_empty() {
            lang = crate::langdetect::normalize_locale(
                &cfg.commit_message.language_check.required_language,
            );
        }
        if lang.is_empty() {
            lang = crate::langdetect::normalize_locale(&cfg.commit_message.locale);
        }
        if lang.is_empty() {
            lang = crate::langdetect::KOREAN.to_string();
        }
