...
use git_warden_config::{BinaryFileConfig, Config};

use crate::staged::get_staged_binary_files;
    ...
    match cfg.policy_for(path).as_str() {
        ...
        "lfs" => {
            if is_lfs_tracked(path) {
                ...
                git_warden_i18n::t!("diff.binary_file_lfs_required", Path = path)
            }
        }
        // "block" and others
        _ => git_warden_i18n::t!("diff.binary_file_error", Path = path),
    }
}
...
mod tests {
    use super::*;
    use git_warden_config::{BinaryFileConfig, BinaryFilePolicyRule};
