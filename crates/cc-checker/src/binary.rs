...
use cc_config::{BinaryFileConfig, Config};
    ...
    match cfg.policy_for(path).as_str() {
        ...
        "lfs" => {
            if is_lfs_tracked(path) {
                ...
                cc_i18n::t!("diff.binary_file_lfs_required", Path = path)
            }
        }
        // "block" and others
        _ => cc_i18n::t!("diff.binary_file_error", Path = path),
    }
}
...
mod tests {
    use super::*;
    use cc_config::{BinaryFileConfig, BinaryFilePolicyRule};
