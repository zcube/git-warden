...
impl FileLanguageRule {
    ...
    pub fn get_locale(&self) -> String {
        let v = crate::langdetect::normalize_locale(&self.locale);
        if !v.is_empty() {
            return v;
        }
        crate::langdetect::normalize_locale(&self.language)
    }
}
...
impl CommentLanguageConfig {
    ...
    pub fn get_locale(&self) -> String {
        ...
        let v = crate::langdetect::normalize_locale(&self.required_language);
        if !v.is_empty() {
            return v;
        }
        crate::langdetect::KOREAN.to_string()
    }
    /// Default false.
...
impl CommitMessageConfig {
    ...
    pub fn coauthor_should_remove(&self, email: &str) -> bool {
        ...
        for pattern in BUILTIN_AI_COAUTHOR_PATTERNS {
            if crate::pathutil::fnmatch(pattern, &email_lower) {
                return true;
            }
        }
        for pattern in &self.coauthor_remove_emails {
            let low = pattern.trim().to_lowercase();
            if crate::pathutil::fnmatch(&low, &email_lower) {
                return true;
            }
        }
        false
