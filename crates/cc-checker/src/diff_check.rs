...
use cc_comment::{Comment, Kind};
use cc_config::Config;
use cc_directive::CommentState;
...
pub fn check_diff(
    ...
    for diff in diffs {
        ...
        if !cc_gitdiff::has_extension(&diff.path, &extensions) {
            continue;
        }
        ...
        let parser = match cc_comment::get_parser(&diff.path) {
            Some(p) => p,
            None => continue,
        };
...
pub(crate) fn resolve_file_lang(path: &str, cfg: &Config) -> String {
    for rule in &cfg.comment_language.file_languages {
        if cc_pathutil::matches_any(path, std::slice::from_ref(&rule.pattern)) {
