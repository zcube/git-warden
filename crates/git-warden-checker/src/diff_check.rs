...
use git_warden_comment::{Comment, Kind};
use git_warden_config::Config;
use git_warden_directive::CommentState;
...
pub fn check_diff(
    ...
    for diff in diffs {
        ...
        if !git_warden_gitdiff::has_extension(&diff.path, &extensions) {
            continue;
        }
        if git_warden_pathutil::matches_any(&diff.path, &ignore_patterns) {
            continue;
        }

        let parser = match git_warden_comment::get_parser(&diff.path) {
            Some(p) => p,
            None => continue,
        };

        let staged_content = match git_warden_gitdiff::get_staged_content(&diff.path) {
            Ok(c) => c,
            Err(_) => continue,
        };
...
pub(crate) fn resolve_file_lang(path: &str, cfg: &Config) -> String {
    for rule in &cfg.comment_language.file_languages {
        if git_warden_pathutil::matches_any(path, std::slice::from_ref(&rule.pattern)) {
