...
use crate::charset::AmbiguousTable;
    ...
    R: Fn(&str) -> Result<Vec<u8>, String>,
{
    ...
    let tables: Vec<&AmbiguousTable> = if check_ambiguous {
        crate::charset::tables_for_locale(&cfg.encoding.locale)
    } else {
        Vec::new()
    };
    ...
    for path in files {
        ...
        for (line_no, line) in content.split('\n').enumerate() {
            for r in line.chars() {
                if check_invisible && crate::charset::is_invisible(r) {
                    let mut name = crate::charset::invisible_name(r).to_string();
                    if name.is_empty() {
                ...
                if check_ambiguous {
                    if let Some(confused_with) = crate::charset::is_ambiguous(r, &tables) {
                        errs.push(crate::t!(
