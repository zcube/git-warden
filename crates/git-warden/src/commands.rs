...
fn load_cfg(g: &Globals) -> Result<Config, CmdError> {
    git_warden_core::config::load(&resolve_config_file_path(&g.config_file))
...
pub fn cmd_run(g: &Globals, format: &str, only: &[String]) -> Result<(), CmdError> {
    ...
    let files = git_warden_core::checker::get_tracked_files()
...
pub fn cmd_diff(
    ...
    let spec = git_warden_core::gitdiff::spec_from_args(args, staged).map_err(CmdError::Msg)?;
    git_warden_core::gitdiff::set_spec(spec);
    ...
    let diffs = git_warden_core::gitdiff::get_staged_diff().map_err(CmdError::Msg)?;
    let cfg = Arc::new(cfg);
    let with_guide = guide_enabled(g, &cfg);
...
pub fn cmd_msg(g: &Globals, msg_file: &str, fix: bool) -> Result<(), CmdError> {
    ...
    if fix {
        let result = git_warden_core::checker::fix_msg(&cfg, &content);
    ...
    let errs = git_warden_core::checker::check_msg(&cfg, &content);
    if !errs.is_empty() {
        for e in &errs {
            eprintln!("{e}");
        }
...
pub fn cmd_push(g: &Globals, range: &str) -> Result<(), CmdError> {
    ...
    for r in &commit_ranges {
        match list_push_commit_hashes(r) {
            Ok(hashes) => {
                for hash in hashes {
                    match get_push_commit_message(&hash) {
                        Ok(msg) => {
                            for e in git_warden_core::checker::check_msg(&cfg, msg.as_bytes()) {
            ...
            Err(err) => eprintln!(
                "{}",
                git_warden_core::t!("cmd.push.warn_list_failed", Range = r, Error = err)
            ),
        }
    }
    ...
    for line in input.lines() {
        ...
        if is_push_zero_sha(remote_sha) {
            ...
            if base.is_empty() {
                eprintln!(
                    "{}",
                    git_warden_core::t!("cmd.push.warn_no_base", Ref = parts[0])
                );
                continue;
            }
...
fn prepare_msg_header_line() -> String {
    format!("# {}", git_warden_core::t!("cmd.prepare_msg.hint_header"))
}
...
fn prepare_msg_hint(cfg: &Config) -> String {
    ...
    if cm.conventional_commit.is_enabled() {
        lines.push(git_warden_core::t!("cmd.prepare_msg.hint_format"));
        lines.push(git_warden_core::t!(
    ...
    if cm.language_check.is_enabled() {
        ...
        if lang != git_warden_core::langdetect::ANY {
            lines.push(git_warden_core::t!(
    ...
    if cm.is_no_ai_coauthor() {
        lines.push(git_warden_core::t!("cmd.prepare_msg.hint_coauthor"));
    }
    if lines.is_empty() {
        return String::new();
    }
...
fn language_display_name(lang: &str) -> String {
    ...
    let name = git_warden_core::i18n::translate(&key, &[]);
    if name == format!("[{key}]") {
        lang.to_string()
    } else {
        name
...
pub fn cmd_fix(g: &Globals, dry_run: bool) -> Result<(), CmdError> {
    ...
    if files.is_empty() {
        println!("{}", git_warden_core::t!("cmd.fix.no_staged_files"));
        return Ok(());
    }
    ...
    for path in &files {
        let content = match std::fs::read(path) {
            ...
            Err(e) => {
                eprintln!(
                    "{}",
                    git_warden_core::t!("cmd.fix.warn_read_failed", Path = path, Error = e)
                );
                continue;
            }
        };
        if git_warden_core::encoding::is_binary(&content) {
            continue;
        }
        let result = git_warden_core::checker::fix_file_content(&cfg, &content);
        if !result.needs_fixing() {
            continue;
        }
        println!("{path}:");
    ...
    if fixed_count == 0 {
        println!("{}", git_warden_core::t!("cmd.fix.no_issues"));
    } else if dry_run {
        println!(
            "{}",
            git_warden_core::t!("cmd.fix.dry_run_summary", Count = fixed_count)
        );
    } else {
        println!(
            "{}",
            git_warden_core::t!("cmd.fix.fixed_summary", Count = fixed_count)
        );
    }
    Ok(())
}
...
// ── version ──
pub fn cmd_version() {
    println!("git-warden {}", crate::version::version());
    println!(
        "{}",
        git_warden_core::t!("version.commit", Value = crate::version::commit())
    );
    println!(
        "{}",
        git_warden_core::t!("version.build_time", Value = crate::version::build_time())
    );
}
...
pub fn cmd_validate(g: &Globals) -> Result<(), CmdError> {
    ...
    let cfg = git_warden_core::config::load(&path).map_err(CmdError::Msg)?;
    let mut warnings = git_warden_core::config::validate(&cfg, &path);
    ...
    warnings.extend(git_warden_core::config::validate_schema_file(&path));
    if warnings.is_empty() {
        println!("{}", git_warden_core::t!("validate.config_valid"));
        return Ok(());
    }
    for w in &warnings {
        eprintln!("{w}");
    }
    Err(CmdError::Msg(git_warden_core::t!(
...
pub fn cmd_migrate(g: &Globals, dry_run: bool) -> Result<(), CmdError> {
    ...
    let result = git_warden_core::config::schema::migrate(&data)
        .map_err(|e| CmdError::Msg(git_warden_core::t!("migrate.failed", Error = e)))?;

    if result.detected_version == git_warden_core::config::schema::Version::Current {
        println!(
            "{}",
            git_warden_core::t!("migrate.already_current", Path = path)
        );
        return Ok(());
    }
    println!(
    ...
    for desc in &result.applied {
        println!("{}", git_warden_core::t!("migrate.change", Desc = desc));
    }
    if dry_run {
        println!("{}", git_warden_core::t!("migrate.dry_run_header"));
        print!("{}", String::from_utf8_lossy(&result.data));
        return Ok(());
    }
    std::fs::write(&path, &result.data)
        .map_err(|e| CmdError::Msg(git_warden_core::t!("migrate.save_failed", Error = e)))?;
    println!("{}", git_warden_core::t!("migrate.saved", Path = path));
    Ok(())
}
